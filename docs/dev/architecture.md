...
### 4. Web UI Rendering
FastAPI serves HTML templates using `Jinja2Templates`. Live updates are **pushed** (D38): the dashboard's default **Live** mode opens an `EventSource` on **`GET /api/events`**, an SSE stream that delivers the full `/api/state` snapshot on connect and again whenever hub state changes. Change detection is the in-process **`StateNotifier`** (version counter + `asyncio.Condition`): the `ActivityTracker` bumps it after every MCP tool call, each mutating operator REST endpoint bumps it (as does `/api/peek`, which moves `last_seen`), and the background sweeper bumps only when a pass actually changed rows. Pushes are debounced (250 ms) so bursts coalesce; idle streams carry keepalive comments (20 s). Client-side, vanilla JS re-renders **only the panels whose slice of the snapshot changed** (per-panel JSON fingerprints), so an idle hub means zero re-renders — the old every-2-s full rebuild (and its jitter) is gone. Pushes arriving while a modal is open are held and applied on close; uptime ticks locally between pushes. `/api/state` remains for the first paint and as the polling fallback (`EventSource` auto-reconnects; after repeated failures the page drops to 2 s polling, and the Refresh dropdown still offers manual intervals/pause). The agents table, message queue, **Activity panel** (in-memory ring buffer, D22), and Job Board all render from the same snapshot. The template is generated with the **`frontend-design`** skill (Tailwind-CDN + vanilla JS, no build step). All agent-controlled fields are passed through an `escapeHtml()` helper before `innerHTML` (stored-XSS defense — see §6 / D18), and the page carries a `Content-Security-Policy` meta.
...
In addition to the loopback bind, the server **validates the HTTP `Origin` header** on `/mcp` (D18): requests with no `Origin` (non-browser CLI clients like Claude Code / Antigravity) pass, but a request carrying a non-localhost `Origin` is rejected. This is the MCP spec's mandated DNS-rebinding defense — it stops a malicious web page in the user's browser from POSTing to `http://localhost:8000/mcp`. It complements, and does not replace, the `127.0.0.1` bind. Implemented as the `OriginValidationMiddleware` — since D38 a **pure ASGI** wrapper (not Starlette's `BaseHTTPMiddleware`, which can buffer streaming responses and delay disconnect propagation; both the `/mcp` streamable-HTTP transport and the `/api/events` SSE stream flow through it). **As hardened 2026-06-18 (D18 note):** the middleware covers `/mcp` **and** `/api/*`, does an **exact `urlparse` host check** (not a substring), validates the **`Host`** header too (DNS-rebinding / CVE-2026-48710), and — because browsers omit `Origin` on same-origin GETs — falls back to **`Sec-Fetch-Site`** for missing-`Origin` requests (allow `same-origin`/`none`, block `cross-site`). The dashboard's own stored-XSS defense (`escapeHtml` + CSP, §4) rounds out the browser-facing hardening.
