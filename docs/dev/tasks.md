...
> 1163/1200 @ 62.4 calls/s (vs 76.1 round-1) with 37 "Recipient unknown" send failures —
> **diagnosed & FIXED same day**: the errors were a harness startup race (`--preregister` flag
> added), and the throughput cost bisected to D37's second-connection-per-completion, now inlined
> into `complete_message`'s held connection (see resolved NOW item below; restart the live hub to
> load it); new `broadcast_stress.py` (`3916393`): caps atomic
...
- [x] **NOW — stress-round-2 HTTP baseline drift.** RESOLVED (2026-07-12, same day — full story in the newest `sessions.md` entry). **(1) "Recipient unknown" = harness startup race, not a hub change**: the reject-unknown guard predates round 1 (present at `060e77d`); every error hits `op=0` (worker *i*'s first send racing worker *i+1*'s first register), count wildly timing-sensitive (7/41/0 across identical runs). `http_loadtest.py` gained op-index error logging + a `--preregister` barrier flag → 1200/1200, 0 errors; **use `--preregister` for future baselines.** **(2) Throughput: worktree bisect (identical harness + machine) pinned the real cost to D37** — `_complete_offer_on_task_success` opened a *second* `_connect()` (new thread + WAL handshake) on every task completion, a 0-row no-op for ordinary tasks: R1 `060e77d`=76.3, pre-D35=78.7, D35=76.7, D36=77.1, **D37=64.8** calls/s. Fixed by inlining the offer-flip UPDATE into `complete_message`'s already-held connection (helper deleted; the `status='assigned'` duplicate-completion guard kept; test reworked to assert through `complete_message`). `pytest` 67/67. Post-fix HEAD 69–71 vs D36 remeasured 68.3 *back-to-back* → **residual drift is run noise** (±5–9 calls/s on this box under live-hub load); antigravity-2's 62.4 was likely depressed by concurrent 3-agent activity. **Writer-contention baseline RECORDED (2026-07-12, post-fix):** canonical params (`--claimers 32 --messages 2000 --writers 24 --writer-ops 200`), live hub running in background: **fixed HEAD `5108da7` = 51 ops/s** (4800/4800, 0 lock errors); round-1 code `060e77d` same evening = 52 ops/s → **no writer regression at HEAD**; antigravity-2's 41 ops/s was pre-fix (D37's extra `_connect` sat in the enqueue→claim→complete loop) + 3-agent load. Claims-scenario speed is load-noisy (747–1249 claims/s across the same evening) — treat correctness (0 double-claims) as the gate, throughput as indicative. Round 3 compares against 51 ops/s. *(Live hub restarted onto the fixed code same day.)*
