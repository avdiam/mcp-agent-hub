...
- `docs/dev/specs.md` — system components, the 14 MCP tools, dashboard, and storage schema.
...
- **Broadcast is one-to-many, ack-less, flood-capped (D33/AHB-1 P1):** the 10th tool `broadcast_message` (→ `db.broadcast`) fans a **`kind="announcement"`** to every non-offline agent **including the sender** (BD5), skipping explicitly-offline peers. Announcements are ack-less (`NO_ACK_KINDS = {result, failure, announcement, offer_update}` — auto-complete on claim, never `reply`/`fail`). Per-sender flood caps (cooldown + hourly + payload/subject size + recipient ceiling) are enforced in `db.broadcast` against the durable `broadcasts` audit table; a violation delivers nothing and returns `{ok: false, error}`. **Late joiners catch up on register (D35/AHB-1 P2):** `register_agent` queues any broadcast from the last 24h (`BROADCAST_CATCHUP_WINDOW`) the registrant has no message row for — dedupe is structural (`session_id = broadcast_id`), so re-registering never re-delivers; the dashboard can broadcast as `operator` via `POST /api/broadcast` (same caps, no echo). Unclaimed announcements are swept by the extended TTL sweep (D24/AHB-1).
- **The job board is a poster-picks auction over the normal task machinery (D36/AHB-2):** `post_offer` broadcasts an advert under the poster's own flood caps (all-or-nothing — caps reject → no offer); `claim_offer` claims accumulate (no window, one pending claim per agent per offer) and notify the poster via ack-less **`kind="offer_update"`**; `resolve_offer('select')` assigns and **auto-sends the payload as a normal `kind="task"`** to the winner (`session_id = offer_id` — result/failure fan back to the poster on the same session), `'withdraw'` takes it down. A failed assignment **re-opens** the offer within its TTL (default 24h; `expire_offers` sweeps). The offer **row** is the source of truth — never `reply`/`fail` an `offer_update`. `list_offers` browses the board; the dashboard's Job Board panel is read-only.
