...
# ... register the 8 @mcp.tool functions and mcp.add_middleware(AgentTracker()) ...
...
A single FastMCP middleware (`on_call_tool` hook) centralizes concerns that would otherwise be copy-pasted into all 8 tools (see `design-decisions.md`, D14):
...
* `upsert_agent(id, capabilities)`, `get_all_agents()`, `set_agent_offline(id)`, `touch_last_seen(id)`
* `enqueue_message(...)`
* `claim_pending(agent_id)` — the atomic `UPDATE ... RETURNING` claim
* `reclaim_stale(visibility_timeout)` — reverts unacked `in_progress` back to `pending`
* `complete_message(id, response)`, `fail_message(id, error)`, `get_status(message_id)`
