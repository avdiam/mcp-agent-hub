...
### 4. Web UI Rendering
FastAPI serves HTML templates using `Jinja2Templates`. To provide live updates without a React/WebSocket setup, the dashboard runs vanilla JavaScript that polls `/api/state` every 2 seconds and refreshes the agents table, the message queue, and an **Activity panel** fed by an in-memory ring buffer of recent tool calls (D22). `/api/state` caps the number of returned messages and also returns the recent activity events + header stats (uptime, total messages). The template is generated with the **`frontend-design`** skill (Tailwind-CDN + vanilla JS, no build step).
